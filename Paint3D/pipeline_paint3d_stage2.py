...
sd_cfg = OmegaConf.load('Paint3D/controlnet/config/UV_based_inpaint_template.yaml')
UVInpaint_cnet = inpaintControlNet(sd_cfg.inpaint) # ok
UVtile_cnet = img2imgControlNet(sd_cfg.img2img) # ok
...
def UV_tile(sd_cfg, cnet, mesh_model, outdir,):
    mesh_model.export_mesh(outdir, export_texture_only=False)
    albedo_path = os.path.join(outdir, f"albedo.png")
    UV_pos = mesh_model.UV_pos_render()
    UV_pos_path = os.path.join(outdir, f"UV_pos.png")
    utils.save_tensor_image(UV_pos.permute(0, 3, 1, 2), UV_pos_path)
...
def retexture_stage2(output_dir, prompt, obj_path, texture_path):
    opt = edict(
        sd_config='Paint3D/controlnet/config/UV_based_inpaint_template.yaml',
