...
class txt2imgControlNet():
    def __init__(self, config, torch_dtype=torch.float16):
        ...
        if config.ip_adapter_image_path:
            ...
            if config.load_ip_adapter_weights:
                pipe.unet._load_ip_adapter_weights(torch.load(config.ip_adapter.path, map_location="cpu"))
        pipe.scheduler = EulerAncestralDiscreteScheduler.from_pretrained(config.sd_model_key, subfolder="scheduler")
        # pipe.scheduler = DPMSolverMultistepScheduler.from_pretrained(config.sd_model_key, subfolder="scheduler")
        pipe.safety_checker = None
        pipe.requires_safety_checker = False
        # pipe.enable_xformers_memory_efficient_attention()
        pipe._progress_bar_config = {"leave": False}
        self.pipe = pipe
    ...
    def infernece(self, config):
        ...
        with self.lock:
            res_image = self.pipe(config.prompt,
                                  ...
                                  region_prompt=['front view, ' + config.prompt, 'back view, ' + config.prompt],
                                  region_negative_prompt=['back view, side view, ' + config.prompt + config.negative_prompt, 'front view, side view, ' + config.prompt + config.negative_prompt],
                                  image=control_img,
