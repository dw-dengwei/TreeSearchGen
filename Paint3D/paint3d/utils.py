...
def odcr(f_img, f_txt):
    if isinstance(f_img, list):
        print(f"img is list: {len(f_img)}")
    if isinstance(f_img, torch.Tensor):
        print(f"img is tensor: {f_img.shape}")
    

    if isinstance(f_txt, list):
        print(f"txt is list: {len(f_txt)}")
    if isinstance(f_txt, torch.Tensor):
        print(f"txt is tensor: {f_txt.shape}")
